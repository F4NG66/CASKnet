...
ax_a.set_title(f"Sequence-level detection rate (n = {n_total:,} sequences)", fontweight="bold", pad=14)
ax_a.set_xlim(0, 105)
...
ax_b.set_title(f"Position-level agreement of high-attention sites (n = {pos_total:,})", fontweight="bold", pad=14)
ax_b.set_xlim(0, 105)
...
for tick, lbl in zip(ax_b.get_yticklabels(), pos_labels):
    tick.set_fontweight("bold")
    tick.set_color(GREEN_DARK if lbl == "Matched" else SUBTLE)
...
ax_d.set_ylim(0, 102)
ax_d.set_title("Cumulative first matched detection position", fontweight="bold", pad=14)
