...
def MySelf_collate_new(batch: List[Tuple[torch.Tensor, torch.Tensor]]):
    embeddings = [item[0] for item in batch]
    label = torch.tensor([item[1] for item in batch])
    embeddings = pad_sequence(embeddings, batch_first=True)
    seq = [item[2] for item in batch]
    name = [item[3] for item in batch]
    return embeddings, label, seq, name
...
def extract_info(subset):
    records = []
    for i in range(len(subset)):
        _, label, seq, pid = subset[i]
        records.append({
            "UniprotID": pid,
            "Sequence": seq,
            "Label": int(label)
...
for fold, (train_idx, val_idx) in enumerate(kf.split(range(len(xlsxProteinDataset))), 1):
    print(f"Training fold {fold}")
    train_dataset = Subset(xlsxProteinDataset, train_idx)
    val_dataset = Subset(xlsxProteinDataset, val_idx)

    train_df = extract_info(train_dataset)
    val_df = extract_info(val_dataset)
    ...
    if weighted:
        ...
        for idx in range(len(train_dataset)):
            seq_tensor, label, _ ,_= train_dataset[idx]
        ...
        weights = [2.25 if l < 25 and lbl == 0 else 1.0 for l, lbl in zip(lengths, labels)]
    ...
    val_loader = DataLoader(val_dataset, batch_size=Config.batch_size,collate_fn = MySelf_collate_fullEmbed )
    
    Save_dataloader = DataLoader(xlsxProteinDataset, batch_size=1 ,collate_fn = MySelf_collate_new)
    # Define model
    model = KAN([4*64, 64, 2])
    ...
    for epoch in range(20):
        ...
        if val_loss < best_val_loss:
            ...
            best_metrics = {"val_loss": val_loss, "val_accuracy": val_accuracy}  # Save best metrics
    ...
    #save_attention_fasta(
        #dataloader=Save_dataloader,
        #model=model,
        #vocab=vocab,
        #save_path="./Result/attention_projection15.fasta",
        #threshold=0.15,
        #device="mps"
    #)

    #interval_output_file = f"log/{fold}CASKnet.txt"
    if weighted:
        interval_output_file = f"5cv/{fold}CASKnet_weighted_2.5.txt"
        #interval_output_file = "final"
        calculate_interval_metrics(val_loader, model, Config, output_file=interval_output_file)
    else:
        interval_output_file = f"5cv/{fold}CASKnet.txt"
        calculate_interval_metrics(val_loader, model, Config, output_file=interval_output_file)

    suffix = "CASKnet_weighted" if weighted else "CASKnet"
    overall_file = f"metrics_5cv/{fold}_{suffix}_overall.txt"
    summary_file = f"metrics_5cv/{suffix}_overall_summary.txt"
    overall = compute_overall_metrics(
        val_loader, model, Config, overall_file, model_kind="casknet"
    )
    append_summary(summary_file, fold, overall)

    bylength_file = f"metrics_5cv_bylength/{fold}_{suffix}_bylength.txt"
    bylength_summary = f"metrics_5cv_bylength/{suffix}_bylength_summary.txt"
